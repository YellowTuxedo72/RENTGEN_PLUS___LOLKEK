...
class KeypointDataset(Dataset):
    def __init__(self, cfg):
        ...
        if len(self.data) == 0:
            raise RuntimeError("❌ Нет совпадений image/json")
...
def debug_sample(dataset):
    ...
    for k in range(hm.shape[0]):
        ...
        x = x * (img.shape[1] / hm.shape[2])
        y = y * (img.shape[0] / hm.shape[1])

        plt.scatter(x, y, c="red")

    plt.title("DEBUG: точки разметки")
    plt.show()
